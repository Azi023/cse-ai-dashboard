...
import { Portfolio, Alert } from '../../entities';
...
export class RiskService {
  ...
  async runRiskAnalysis(): Promise<void> {
    ...
    const holdings = await this.portfolioRepo.find({ where: { is_open: true } });
    ...
    for (const holding of holdings) {
      try {
        const risk = await this.computeRiskForHolding(holding, today, tradeMap, totalPortfolioValue);
        if (risk) results.push(risk);
      } catch (err) {
        this.logger.warn(`Risk analysis failed for ${holding.symbol}: ${String(err)}`);
      }
    }

    // Update portfolio_heat_pct on each row
    const totalHeat = results.reduce((s, r) => s + Number(r.position_heat_pct), 0);
    for (const risk of results) {
      risk.portfolio_heat_pct = Math.round(totalHeat * 100) / 100;
      risk.risk_status = totalHeat < 3 ? 'SAFE' : totalHeat < 5 ? 'CAUTION' : 'DANGER';
      await this.riskRepo.save(risk);
    }

    this.logger.log(`Risk analysis complete: ${results.length} positions, total heat ${totalHeat.toFixed(1)}%`);
  }
  ...
  async runForSymbol(symbol: string): Promise<PositionRisk | null> {
    const holding = await this.portfolioRepo.findOne({ where: { symbol, is_open: true } });
    ...
    const totalPortfolioValue = summary?.total_value ?? 0;
    return this.computeRiskForHolding(holding, this.todayStr(), tradeMap, totalPortfolioValue);
  }

  async getPositionRisks(): Promise<PositionRisk[]> {
    const today = this.todayStr();
    const rows = await this.riskRepo.find({ where: { date: today }, order: { symbol: 'ASC' } });
    if (rows.length > 0) return rows;
    // Fall back to most recent
    return this.riskRepo.find({ order: { date: 'DESC', symbol: 'ASC' }, take: 20 });
  }
  ...
  async getPortfolioRiskSummary(): Promise<{
    ...
    const totalHeat = positions.reduce((s, p) => s + Number(p.position_heat_pct), 0);
    const maxLoss = positions.reduce((s, p) => s + Number(p.max_loss_lkr), 0);
    const maxGain = positions.reduce((s, p) => s + Number(p.max_gain_lkr), 0);
    const riskStatus = totalHeat < 3 ? 'SAFE' : totalHeat < 5 ? 'CAUTION' : 'DANGER';
    return { positions, total_heat_pct: Math.round(totalHeat * 100) / 100, risk_status: riskStatus, max_loss_lkr: Math.round(maxLoss), max_gain_lkr: Math.round(maxGain) };
  }
  ...
  getSizing(input: {
    ...
    if (riskPerShare <= 0) return { max_risk_amount: maxRiskAmount, max_shares: 0, actual_shares: 0 };
  ...
  private async computeRiskForHolding(
    ...
    const effectiveEntry = entryPrice + (Number(holding.fees) / Number(holding.quantity));
    ...
    const supportStop = support !== null && atr !== null ? round2(support - 0.5 * atr) :
                        support !== null ? round2(support * 0.995) : null;
    ...
    const riskRewardRatio = downside > 0 ? round2(rewardPerShare / downside) : 0;
    const distToStopPct = round2(((currentPrice - recommendedStop) / currentPrice) * 100);
    const maxLossLkr = round2(sharesHeld * Math.max(0, currentPrice - recommendedStop));
    const maxGainLkr = round2(sharesHeld * Math.max(0, takeProfit - currentPrice));
    const positionHeatPct = totalPortfolioValue > 0 ? round2(maxLossLkr / totalPortfolioValue * 100) : 0;

    const existing = await this.riskRepo.findOne({ where: { date, symbol: holding.symbol } });
